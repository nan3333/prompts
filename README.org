...
- =<word>= means to substitute the variable named _word_ into this position of the prompt.
- =<delim>= means to substitute the delimiter, usually =###= for GPT-3 and ="""= for Codex, into this position of the prompt.
...
- =<:s>= denotes the place where completion happens. Therefore =<:pp>= must always proceed =<:pp>=

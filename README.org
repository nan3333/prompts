...
+ You may also supply variables:
  - =vars=
    - =vars= is a list of variable names
    - The first variable may be captured by selecting some text in =pen.el=
    - The remaining arguments may be captured either with a specified elisp function, or by default
